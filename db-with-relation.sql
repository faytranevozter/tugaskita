...
CREATE TABLE `ajar` ( 
	...
	PRIMARY KEY (`ajar_id`),
	FOREIGN KEY (`ajar_dosen_id`) REFERENCES `dosen`(`dosen_id`),
	FOREIGN KEY (`ajar_matakuliah_id`) REFERENCES `matakuliah`(`matakuliah_id`)
);
...
CREATE TABLE `krs` ( 
	...
	PRIMARY KEY (`krs_id`),
	FOREIGN KEY (`krs_mahasiswa_id`) REFERENCES `mahasiswa`(`mahasiswa_id`),
	FOREIGN KEY (`krs_ajar_id`) REFERENCES `ajar`(`ajar_id`)
);
...
CREATE TABLE `tugas` ( 
	...
	FOREIGN KEY (`tugas_ajar_id`) REFERENCES `ajar`(`ajar_id`)
);
...
CREATE TABLE `kumpul` ( 
	...
	FOREIGN KEY (`kumpul_tugas_id`) REFERENCES `tugas`(`tugas_id`)
);
...
CREATE TABLE `nilai` ( 
	...
	FOREIGN KEY (`nilai_kumpul_id`) REFERENCES `kumpul`(`kumpul_id`)
);
